...
use libra_secure_storage::{Error as StorageError, KVStorage, Storage, Value};
use libra_network_address::{NetworkAddress, encrypted::{EncNetworkAddress, Key, KeyVersion}};
...
impl Encryptor {
    ...
    pub fn encrypt(
        ...
        let keys = self.read()?;
        let key = keys.keys.get(&keys.current).ok_or(Error::VersionNotFound(keys.current))?;
        ...
        let encrypted_key = EncNetworkAddress::encrypt(raw_addr, key, keys.current, &account, seq_num, 0);
        lcs::to_bytes(&encrypted_key).map_err(|e| e.into())
    }

    pub fn decrypt(
        &self,
        encrypted_network_address: &Vec<u8>,
        account: AccountAddress,
        ...
        let key = keys.keys.get(&keys.current).ok_or(Error::VersionNotFound(enc_addr.key_version()))?;
        // TODO(davidiw) soon we'll take in a set, for now we assume there's only one addr
        let raw_addr = enc_addr.decrypt(key, &account, 0).map_err(|e| Error::DecryptionError(account, e.to_string()))?;
    ...
    fn read(&self) -> Result<ValidatorKeys, Error> {
        let data = self.storage.get(VALIDATOR_NETWORK_ADDRESS_KEYS)?.value.bytes()?;
        lcs::from_bytes(&data).map_err(|e| e.into())
    }

    fn write(&mut self, keys: &ValidatorKeys) -> Result<(), Error> {
        let data = lcs::to_bytes(keys)?;
        self.storage.set(VALIDATOR_NETWORK_ADDRESS_KEYS, Value::Bytes(data)).map_err(|e| e.into())
    }
}

#[derive(Debug, Deserialize, Serialize)]
struct ValidatorKeys {
    keys: HashMap<KeyVersion, Key>,
    current: KeyVersion,
}
...
mod tests {
    ...
    fn e2e() {
        ...
        encryptor.add_key(0, key.clone()).unwrap();
        encryptor.set_current_version(0).unwrap();
        rng.fill_bytes(&mut key);
        encryptor.add_key(1, key.clone()).unwrap();
        encryptor.set_current_version(1).unwrap();
        rng.fill_bytes(&mut key);
        encryptor.add_key(4, key.clone()).unwrap();
